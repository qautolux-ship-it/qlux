...
async function updateFrontPageBonus(user) {
  const bonusContainer = document.getElementById("frontBonus");
  const bonusText = document.getElementById("frontBonusText");
  const bonusAction = document.getElementById("frontBonusAction");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    updateNavbar(user);
    await updateFrontPageBonus(user);
    if (user) {
      await updateBudgetPlaceholder();
    }
  });
